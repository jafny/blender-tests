"""Render stills or flythrough frames.
    ...
    python3 render.py stills  [--samples 128] [--res 1920 1080] [--pct 100] [--only NAME ...] [--out DIR]
    python3 render.py anim    [--samples 32]  [--res 1280 720]  [--start 1] [--end 300] [--out DIR]
    python3 render.py frame N [--samples 16] ...   (single flythrough frame, for previews)
"""
import os
import sys
import time
...
def main():
    ...
    ap.add_argument("--outlines", choices=["freestyle", "ink", "none"], default="freestyle")
    ap.add_argument("--save", default=None, help="also save .blend here")
    argv = sys.argv[sys.argv.index("--") + 1:] if "--" in sys.argv else sys.argv[1:]
    a = ap.parse_args(argv)
    scene, cams, fly = build_scene.build(props_=not a.no_props)
    anim = a.mode != "stills"
    samples = a.samples or (32 if anim else 128)
    res = tuple(a.res) if a.res else ((1280, 720) if anim else (1920, 1080))
    lighting.render_settings(scene, samples=samples, res=res, pct=a.pct, animation=anim, outlines={"ink": True, "freestyle": "freestyle", "none": False}[a.outlines])
    if a.save:
        bpy.ops.wm.save_as_mainfile(filepath=os.path.abspath(a.save))
    out = a.out or os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "renders",
                                "stills" if a.mode == "stills" else "frames")
